...
""",
        ...
        self.assertEqual(function.cpp.doc.brief, "Forward declaration docs.")
        self.assertEqual(function.cpp.doc.parameters["value"], "Value from the forward declaration.")
        self.assertIsNone(function.cpp.doc.returns)
        self.assertEqual(function.parameters[0].cpp.doc, "Value from the forward declaration.")
...
""",
    ...
    def test_parse_headers_keep_first_attached_comment_for_repeated_forward_class_declarations(self) -> None:
        sources = {
            "a.hpp": """
namespace demo {
...
""",
        ...
        result_last = _parse_headers_from_sources(
            sources,
            header_order=["a.hpp", "b.hpp"],
            parser_config=ParserConfig(
                auto_detect_toolchain=False,
                cxx_standard="c++20",
                comment_conflict_policy="last",
            ),
        )

        widget = result.module.declarations.namespaces[0].declarations.classes[0]
        widget_last = result_last.module.declarations.namespaces[0].declarations.classes[0]

        self.assertEqual(len(widget.cpp.location.declarations), 2)
        self.assertEqual(widget.cpp.comment, "/// First forward docs.")
        self.assertEqual(widget.cpp.doc.brief, "First forward docs.")
        self.assertEqual(widget_last.cpp.comment, "/// First forward docs.")
        self.assertEqual(widget_last.cpp.doc.brief, "First forward docs.")
        self.assertFalse(
            any("Conflicting parsed comments" in warning for warning in result.warnings),
            msg=f"Did not expect comment-conflict warning, got: {result.warnings}",
        )
